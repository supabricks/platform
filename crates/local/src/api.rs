...
pub enum Action {
    ManagedIncremental {
        command: crate::incremental::Command,
    },
    ManagedCapture {
    ...
    CatalogDatasets {
        command: crate::catalog::datasets::Command,
    },
    CatalogPublication {
...
pub fn capabilities(binding: &Binding) -> Value {
    json!({"api":"supabricks.local", "api_version":VERSION,"project_id":binding.project_id,"worktree":binding.worktree,
        "postgres_major":17,"features":{"branching":true,"stable_connections":true,"wake_on_connect":true,"automatic_idle_suspend":false,"analytics":true,"unpublished_exports":true,"atomic_snapshots":true,"managed_snapshot_scheduling":true,"sync_controls":1,"local_durable_capture":true,"local_incremental_epochs":true,"incremental_triggered":true,"continuous_sync":true,"sync_event_triggers":false,"ingestion":true,"notebook_environments":true,"notebook_packages":true,"logical_data_packages":true,"catalog_metadata":true,"catalog_datasets":true},
...
pub(crate) fn handle(
    ...
    let (key, mutation) = match action {
        Action::ManagedIncremental { command } => {
            store.binding_context(binding)?;
            if matches!(command, crate::incremental::Command::Apply { .. }) {
                if cell.is_none() {
                    return Err(invalid("materialization requires a native cell"));
                }
                let (_, worker) = crate::installation::analytical_worker(store.root())?;
                if !worker.with_file_name("incremental_worker.py").is_file() {
                    return Err(invalid("install the SY03 analytical runtime"));
                }
            }
            return store.incremental_command(binding.project_id, command);
        }
        ...
        Action::ManagedSnapshots { command } => {
            let incremental = match &command {
                crate::sync::Command::Create { config, .. }
                | crate::sync::Command::Update { config, .. } => config.incremental(),
                crate::sync::Command::Resume { id, .. } => {
                    store.sync_policy(project, *id)?.config.incremental()
                }
                _ => false,
            };
            if incremental {
                if cell.is_none() {
                    return Err(invalid("incremental sync requires a native cell"));
                }
                let (_, worker) = crate::installation::analytical_worker(store.root())?;
                if !worker.with_file_name("incremental_worker.py").is_file() {
                    return Err(invalid("install the SY05 analytical runtime"));
                }
            }
            return crate::sync::handle(store, binding, command);
        }
        Action::CatalogDatasets { command } => {
            return crate::catalog::datasets::handle(store, binding, command);
        }
