...
pub(crate) fn handle(
    ...
    let (key, mutation) = match action {
        ...
        Action::ManagedSnapshots { command } => {
            if matches!(&command,crate::sync::Command::Create{config,..}|crate::sync::Command::Update{config,..} if config.triggered())
            {
                if cell.is_none() {
                    return Err(invalid("triggered sync requires a native cell"));
                }
                let (_, worker) = crate::installation::analytical_worker(store.root())?;
                if !worker.with_file_name("incremental_worker.py").is_file() {
                    return Err(invalid("install the SY04 analytical runtime"));
                }
            }
