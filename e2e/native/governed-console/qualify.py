...
def main():
    ...
    base=f'https://127.0.0.1:{port()}';issuer=base+'/realms/uc097';origin=f'{"https" if args.tls else "http"}://127.0.0.1:{port()}';callback=origin+'/auth/v1/callback'
    ...
    try:
        ...
        subprocess.run(['docker','run',*(['--pull=never'] if args.exact_installed else []),'-d',*network,'--name',name,'--memory=1g','--cpus=2','--pids-limit=256','--env-file',str(root/'keycloak.env'),'-v',str(root/'realm.json')+':/opt/keycloak/data/import/realm.json:ro','-v',str(root/'cert.pem')+':/tls/cert.pem:ro','-v',str(root/'key.pem')+':/tls/key.pem:ro',pin,'start-dev','--import-realm','--http-enabled=false',*(['--https-port='+base.rsplit(':',1)[1]] if os.environ.get('SUPABRICKS_QUALIFIER_CONTAINER') else []),'--https-certificate-file=/tls/cert.pem','--https-certificate-key-file=/tls/key.pem','--hostname='+base],check=True,capture_output=True)
        started=True
        ...
        if args.exact_installed:
            assert args.uc_runtime.resolve()==args.release.resolve()/'share/unity-catalog'
        else:
        ...
        if args.tls:
            for filename in ('cert.pem','key.pem'):
                shutil.copyfile(root/filename,root/('ingress-'+filename));(root/('ingress-'+filename)).chmod(0o600)
            (root/'ingress.json').write_text(json.dumps(dict(listen=origin.removeprefix('https://'),origin=origin,certificate=str(root/'ingress-cert.pem'),private_key=str(root/'ingress-key.pem'),qualification=None)))
            ingress=['--ingress',str(root/'ingress.json')]
        log=(root/'browser.log').open('w');browser=subprocess.Popen([str(cell.binary),'console','--governed','--provider','keycloak','--redirect',callback,*ingress,'--data-dir',str(cellroot)],stdout=log,stderr=log)
    ...
    evidence.update(binary_sha256=hashlib.sha256(args.binary.read_bytes()).hexdigest(),
        console_manifest_sha256=hashlib.sha256(((args.release/'share/console/console.json') if args.exact_installed else (args.console/'dist/console.json')).read_bytes()).hexdigest(),
        execution_config_sha256=hashlib.sha256(args.execution_config.read_bytes()).hexdigest(),
        keycloak=pin,cleanup='PASS',tls=args.tls,exact_installed=args.exact_installed)
    if args.exact_installed:evidence['release_identity']=hashlib.sha256((args.release/'release.json').read_bytes()).hexdigest()
    args.report.write_text(json.dumps(evidence,indent=2)+'\n')
