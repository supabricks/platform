...
def main():
    ...
    try:
        ...
        subprocess.run(['docker','run','--pull=never','-d',*network,'--name',name,'--memory=1g','--cpus=2','--pids-limit=256','--env-file',str(root/'keycloak.env'),'-v',str(root/'realm.json')+':/opt/keycloak/data/import/realm.json:ro','-v',str(root/'cert.pem')+':/tls/cert.pem:ro','-v',str(root/'key.pem')+':/tls/key.pem:ro',pin,'start-dev','--import-realm','--http-enabled=false',*(['--https-port='+base.rsplit(':',1)[1]] if os.environ.get('SUPABRICKS_QUALIFIER_CONTAINER') else []),'--https-certificate-file=/tls/cert.pem','--https-certificate-key-file=/tls/key.pem','--hostname='+base],check=True,capture_output=True)
        started=True
