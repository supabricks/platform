...
def qualify(args):
    ...
    native.argtypes=[ctypes.POINTER(ctypes.c_uint64)]
    def snapshot():
        values=(ctypes.c_uint64*8)();native(values)
        return list(values)
    with tempfile.TemporaryDirectory(prefix='sp02-component-',dir=args.scratch) as tmp:
        spool=Spool(Path(tmp)/'spool',dict(generation='component',decoder_version=2));spool.establish(100,{})
        ...
        captured=spool.captured;stats=groups.stats if groups else None;spool.close()
        ...
        return dict(status='pass',parameters=dict(count=args.count,age_ms=args.age_ms,offered_transactions_s=args.rate,seconds=args.seconds,reader=args.reader,single=args.single),
                    ...
                    durability='DELETE / FULL; native sync totals include pruning',sqlite_version=sqlite3.sqlite_version)
