#!/usr/bin/env python3
"""SP03b immutable diagnostic packages and explicitly labeled factorial ablations."""
import argparse
import hashlib
import json
...
def overlay(repo,base,dest,proof,predecessor=False,binary=None,mode='wal',single=False):
    ...
    if not predecessor:
        for name in ('capture/spool.py','capture/protocol.py','capture/groups.py','capture/wal.py','incremental/storage.py'):
            sources[name]=(repo/'python/analytics'/name).read_bytes()
        if mode=='delete':
            sources['capture/spool.py']=sources['capture/spool.py'].replace(b"journal_mode='wal'",b"journal_mode='delete'")
        if single:
            sources['capture/groups.py']=sources['capture/groups.py'].replace(b'count=32,',b'count=1,')
        assert binary and binary.is_file()
        replace('bin/supabricks',binary.read_bytes())
    ...
    if predecessor:assert sha(dest/'bin/supabricks')==sha(base/'bin/supabricks')
    result=dict(base_release_sha256=original,release_sha256=sha(p),predecessor=predecessor,
        ...
        binary_sha256=sha(dest/'bin/supabricks'),journal_mode='delete' if predecessor else mode,single=single,changes=changes,
...
if __name__=='__main__':
    ...
    p.add_argument('--predecessor',action='store_true');p.add_argument('--binary',type=Path)
    p.add_argument('--journal-mode',choices=('delete','wal'),default='wal');p.add_argument('--single',action='store_true');a=p.parse_args()
    overlay(a.repo.resolve(),a.base.resolve(),a.destination.resolve(),a.proof,a.predecessor,a.binary,a.journal_mode,a.single)
