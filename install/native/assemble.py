...
''')
    ...
    shutil.copytree(ROOT / 'examples/console', destination / 'examples/console')
    shutil.copytree(ROOT / 'examples/projects', destination / 'examples/projects')
    ...
    shutil.copy2(ROOT / 'docs/handbook/governed-server.md', destination / 'GOVERNED-SERVER.md')
    (destination / 'share/governed').mkdir()
    shutil.copy2(ROOT / 'install/native/prepare_governed_runtime.py', destination / 'share/governed/prepare-runtime.py')
    shutil.copy2(ROOT / 'components/execution-runtime.lock.json', destination / 'share/governed/execution-runtime.lock.json')
    ...
    shutil.copy2(ROOT / 'docs/handbook/catalog-metadata.md', destination / 'CATALOG-METADATA.md')
    shutil.copy2(ROOT / 'docs/handbook/catalog-publication.md', destination / 'CATALOG-PUBLICATION.md')
    ...
    if not args.postgres_only:
        ...
        for name in ('pyproject.toml', 'uv.lock'):
            shutil.copy2(destination / 'python/notebooks/environments/base' / name,
                         destination / 'examples/projects/sales-runnable/notebooks/environment' / name)
    if not args.postgres_only:
    ...
    provenance = dict(
        ...
        data_formats=dict(local_catalog=28, runtime_config=2, postgres_major=17, analytical_snapshot=1, incremental_snapshot=2),
