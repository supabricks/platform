...
def collect(directory, revision, console, worker, version):
    ...
    for target in TARGETS:
        ...
        require(source['data_formats']['local_catalog'] == 11 and source['data_formats']['postgres_major'] == 17,
        ...
        baseline = read('release-qualification', 'qualification.json', 10, ('release_identity',))
        require(baseline.get('network_qualification'), 'missing baseline network evidence')
        benchmark = read('release-qualification', 'benchmarks.json', 10, ('release_identity',)) if target == 'linux-x86_64' else baseline
