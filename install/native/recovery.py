...
def qualify(args):
    ...
    report = dict(status='failed', checks=checks, network_qualification=args.network_evidence,
                  limits=['process-failure recovery; no kernel reboot or power-loss qualification',
                          'same-target physical restore and catalog upgrade to the candidate manifest schema; identical engine/dependency inventories'])
...
""")
    ...
    try:
        ...
        cli('branch', 'use', 'main')  # Persist explicit legacy worktree evidence before migration.
        ...
        candidate_schema = json.loads((current_release / 'release.json').read_text())['provenance']['data_formats']['local_catalog']
        ...
        for phase in ['before_migration', 'after_migration', 'runtime_rebound', 'current_activated']:
            ...
            install('new', upgrade=True)
            with sqlite3.connect(data / 'state.sqlite3') as db:
                assert db.execute('PRAGMA user_version').fetchone()[0] == candidate_schema
                assert db.execute('SELECT source_sha256,release_identity FROM catalog_migrations WHERE version=?', (candidate_schema,)).fetchone() == (journal['database_sha256'], identity)
            assert not (data / 'upgrade.json').exists()
        ...
        checks.append(f'catalog 9-to-{candidate_schema} resumes before/after migration and both activation boundaries; old binary refuses migrated root')
        cli('up')
