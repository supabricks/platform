...
def qualify(args):
    ...
    report = dict(status='failed', checks=checks, network_qualification=args.network_evidence,
                  limits=['process-failure recovery; no kernel reboot or power-loss qualification',
                          'same-target physical restore and catalog-9-to-13 upgrade; identical engine/dependency inventories'])
    ...
    try:
        ...
        for phase in ['before_migration', 'after_migration', 'runtime_rebound', 'current_activated']:
            ...
            with sqlite3.connect(data / 'state.sqlite3') as db:
                ...
                assert db.execute('SELECT source_sha256,release_identity FROM catalog_migrations WHERE version=13').fetchone() == (journal['database_sha256'], identity)
            assert not (data / 'upgrade.json').exists()
        ...
        checks.append('catalog 9-to-13 resumes before/after migration and both activation boundaries; old binary refuses migrated root')
        cli('up')
