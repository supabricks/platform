...
if args.macos_upgrade_policy and (not args.lifecycle or args.target != 'macos-arm64'):
    parser.error('--macos-upgrade-policy is only for macOS lifecycle qualification')
...
for harness in (['lifecycle', 'index'] if args.lifecycle else ['packages'] if args.packages else ['manager', 'kernels']):
    ...
    command = [str(release / 'python/runtime/bin/python3.12'), '-I', '-B',
    ...
    if args.macos_upgrade_policy:
        if harness == 'lifecycle':
            command += ['--macos-upgrade-policy', str(args.macos_upgrade_policy.resolve())]
        else:
            # The resolver/index fixture has no historical live predecessor;
            # isolate its entire process tree from the start.
            command = ['sandbox-exec', '-f', str(args.macos_upgrade_policy.resolve()), *command]
