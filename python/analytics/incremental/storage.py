...
import sqlite3
import time
from capture.spool import CaptureError, canonical, atomic, lsn, MAX_TRANSACTION
...
def journal(config):
    ...
    try:
        ...
        if meta['captured']<target or meta['start']>after:raise CaptureError('source_history_lost')
...
def initialize(config):
    ...
    try:
        for entry in manifest['files']:
            ...
            # Delta's immutable existing data/log files can share an inode. Future
            # commits create new names; neither A01 nor an A02 v1 generation is edited.
            os.link(src,dest)
