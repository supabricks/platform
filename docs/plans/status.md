...
| Open-source Unity Catalog | UC00 merged in platform #56, Linux/macOS qualified (14 checks each); UC01 merged in #57 with both native offline suites qualified; UC02 merged in #58 with both native catalog suites passed; UC03 durable publication implemented with qualification in progress; UC04–UC09 remain planned | [UC00 capability report](../architecture/uc00-catalog-probe.md), [UC00–UC09 plan](unity-catalog-implementation.md): local catalog workflow first; governed access depends on IAM and isolation |
