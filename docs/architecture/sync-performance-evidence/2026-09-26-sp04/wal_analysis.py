...
def analyze_trial(root,result):
    ...
    profile=load(profile_path);totals=dict(prune_calls=0,prune_ms=0,progress_calls=0,progress_ms=0,transactions=0,seconds=0,commit_syncs=0,checkpoint_syncs=0,all_native_syncs=0,checkpoint_calls=0,checkpoint_ms=0,commit_ms=0,commit_sync_ms=0,checkpoint_sync_ms=0,all_native_sync_ms=0,all_native_sync_errors=0)
    feedback_checks=0
    for name,rows in profile['workers'].items():
        ...
        def delta(section,key,field):
            ...
            assert difference>=0
            return difference
        for operation in ('prune','progress'):
            totals[operation+'_calls']+=delta('metrics','capture.spool.'+operation,'calls')
            totals[operation+'_ms']+=delta('metrics','capture.spool.'+operation,'total_ns')/1e6
...
def analyze(root):
    result={}
    for name in ('main','predecessor-controls','candidate-controls'):
