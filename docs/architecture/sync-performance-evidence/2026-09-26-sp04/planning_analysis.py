...
def fullstack(root):
        ...
        for arm,result in pair['results'].items():
            ...
            for name,snapshots in profile['workers'].items():
                ...
                workers.append(dict(stream=name,**costs(last),
                    lifecycle_cpu_seconds=last['cpu_user_s']+last['cpu_system_s'],
                    startup_imports_ms=last['metrics'].get('startup.imports',{}).get('total_ns',0)/1e6,
                    journal_transactions=last['work'].get('apply.journal.transactions',{}).get('total',0),
                    native_sync_calls=sum(v['calls'] for v in (last.get('native_io') or {}).values()),
                    native_sync_ms=sum(v['total_ns'] for v in (last.get('native_io') or {}).values())/1e6))
            ...
            trials.append(dict(arm=arm,cpus=pair['pair']['cpus'],rate=pair['pair']['rate'],repeat=pair['pair']['repeat'],
                ...
                completed_worker_totals={key:sum(r[key] for r in workers) for key in metrics},
    ...
    for cpus,rate,arm in sorted({(r['cpus'],r['rate'],r['arm']) for r in trials}):
        ...
        groups.append(dict(cpus=cpus,rate=rate,arm=arm,
            completed_workers=describe([r['completed_workers'] for r in selected]),
            trial_worker_totals={key:describe([r['completed_worker_totals'][key] for r in selected if key in r['completed_worker_totals']]) for key in metrics},
            trial_worker_medians={key:describe([r['worker_medians'][key] for r in selected if key in r['worker_medians']]) for key in metrics}))
